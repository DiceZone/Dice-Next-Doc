...
| `.bot log on/off` | 开关本群日志功能（指令与记录） |
| `.bot reply on/off` | 开关本群自定义回复与因果规则，与 `.reply on/off` 共用设置 |
| `.bot roll on/off` | 开关内置掷骰、检定、先攻与技能成长 |
| `.bot plugin on/off` | 开关插件指令、消息钩子与规则包自定义指令 |
| `外置开` / `外置关` | 外置模式：停用内置指令，仅保留自定义回复 |

`.bot` 信息可配置页眉 / 页脚（见[系统设置](/manage/settings)）。

四个功能开关默认开启，按「本群 + 当前骰子账号」保存，需与 `.bot on/off` 相同的群管理权限。省略 `on/off` 可查询该功能的设置，也可在末尾加骰子账号定向操作。

`.bot off` 仍是总开关，关闭后 `.log` 和 `.reply` 不再例外放行；已有的定向 @、紧急管理等兼容规则保持不变。四个子开关可以在总开关关闭时调整，但不会唤醒骰子。重新 `.bot on` 不会重置子开关。

插件开关不会修改各个插件的独立启停设置；插件内部掷骰归插件开关控制，不归内置投掷开关控制。人物卡管理、规则选择等非投掷操作不受投掷开关影响。
...
.group +禁用回复 -禁用help → 一次调整多个词条
.group 123456 state     → 查看指定群（校验目标群权限）
.group 123456 +禁用回复 → 调整指定群
.group all +禁用回复    → 当前骰子账号的各群批量设置（骰主或信任≥5）
.group clr              → 重置当前账号的本群功能设置
```

可用群管词条：`停用指令`、`禁用回复`、`禁用jrrp`、`禁用draw`、`禁用me`、`禁用help`、`禁用deck`、`禁用send`。

指定群也可在私聊中操作。当前群的管理员身份不能直接用于其他群；目标群彻底禁用时不能通过这些指令解除。`clr` 不影响其他骰子账号，保留群身份信息、硬锁及未结束的日志，不删除日志内容；旧版共享设置也不会在重置后重新生效。`all` 只接受带正负号的词条，不支持批量清空。
